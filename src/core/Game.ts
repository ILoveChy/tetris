...
export class Game {
    ...
    private switchTeris() {
        ...
        //有可能出问题 当前方块已出现时,就已经何志强的方块重叠了
        if (!TerisRule.canIMove(this._curTeris.shape, this._curTeris.centerPoint, this._exists)) {
            //游戏结束
            this._gameStatus = GameStatus.over;
            clearInterval(this._timer);
            this._timer = undefined;
            this._viwer.onGameOver();
            return;
        }
        this.createNext()
        this._viwer.switch(this._curTeris);
    }
    /**
     * 设置中心点坐标,已达到让该方块出现在区域的顶部中间位置
     * @param width 
     * @param teris 
     */
